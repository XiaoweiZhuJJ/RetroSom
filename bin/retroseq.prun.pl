...
### v2 changes Jul 2013 ###
### 1. lower the cutoff ###
### 2. fix the SR clip reads bug (flag 83 and 147) ###
### 3. return the sequence complexity scores for alignment of the supporting reads ###
### v3 change ###
### remove PE supports with low sequencing quality ###

### no local affnity in exonerate ###
#### higher similarity score for the alingment ###
### higher threshold for mismatches and seq qualitties ###
### max depth to 1000 ###

### checking split reads to allow intact 3'end ###
### realign all discordant paired-end reads ###

### 0916 ###
### local affine ###
### do not count mismatches based on MD tags ###
### add calling for A(alu)L(line1)H(hervk,hervh)S(svae svaf)
#
### 040616 ###
### pipe in exonerate reached system limits, leads to loss of candidate reads ###

### 101016 ###
### print qas in exonerate SR (fot ACA filtering) ###
### fixed SR parsing error in SR ###

### 041917 ###
### fixed PE support reads with both ends mapped to L1 ###
### print anchor sequence for support reads with split-read anchors ###

### 050317 ###
### ADD SA tags to PE reads if there is one ###

### 050517 ###
### Add MAPQ AS and XS tag for PE support ###

### 051617 ###
### Add Anchor information for SR support ###
### lower default ID from 85 to 80 ###

### 092817 ###
### output gap number in the alignment ###
### fix anchor rightmost coordiante ###
### add mapping coordinates for SR anchors ###
...
my $DEFAULT_MAX_DEPTH = 1000;
my $DEFAULT_READS = 1; # 10;
my $DEFAULT_MIN_GENOTYPE_READS = 1; #3;
my $MAX_READ_GAP_IN_REGION = 120;
my $DEFAULT_MIN_CLUSTER_READS = 1; #2;
my $DEFAULT_SR_MIN_CLUSTER_READS = 1; #3;
my $DEFAULT_SR_MIN_UNKNOWN_CLUSTER_READS = 1; #10;
...
my ($i, $discover, $call, $genotype, $bam, $bams, $ref, $eRefFofn, $length, $id, $output, $anchorQ, $region, $input, $reads, $depth, $noclean, $tmpdir, $readgroups, $filterFile, $heterozygous, $orientate, $ignoreRGsFofn, $srmode, $minSoftClip, $srOutputFile, $srInputFile, $callNovel, $refTEs, $excludeRegionsDis, $doAlign, $singleEnds, $help);
...
sub _findCandidates
{
    ...
    my $clipAnchor= qq[$$.clip_anchor.txt];
    my $safile = qq[$$.SAtag];
    ...
    (open (SA, ">>$safile")) || die "cannot open the sa file\n";
    if( keys( %candidates ) > 0 )
    {
        ...
        my $currentChr = '';
        my $readsFound = 0;
        my ($line, $first, %clip, @data, $a_mm, $a_AS, $a_XS);

        (open (CFA, "<$clipFasta")) || die "cannot open the clipfasta file\n";
        (open (CAN, ">$clipAnchor")) || die "cannot open the clip anchor file\n";
        ...
        while( my $sam = <$bfh> )
        {
            ...
            if (exists($clip{$name."\t".$first}))
               {
                ...
                $sam =~ /NM:i:(\d+)/;
                $a_mm = $1;
                ### name flag cigar mismatch mapQ AS XS coord ###
                print CAN "$name\t$flag\t$s[5]\t$a_mm\t$s[4]\t$a_AS\t$a_XS\t$s[3]\n";
               }
            ...
            if( $candidates{ $name } )
            {
                ...
                if( ($flag & $$BAMFLAGS{'1st_in_pair'}) && $candidates{ $name } == 1 || ($flag & $$BAMFLAGS{'2nd_in_pair'}) && $candidates{ $name } == 2 )
                {
                    ...
                    if ($sam =~ /SA/)
                       {
                        for ($sam_i=10; $sam_i <=$#s; $sam_i++)
                           {
                            $SA = $1 if ($s[$sam_i] =~ /^SA:Z:(.+)/);
                           }
                        print SA "$name###$s[2]###$s[3]###$s[5]###$s[1]\t$SA\n";
                       }
                    if( (_sequenceQualityOK( $seq )) && (_checkAvgQuality($seqqual)) )
                    {
                        if( $doAlign ){print $ffh qq[>$name###$s[2]###$s[3]###$s[5]###$s[1]\n$seq\n];}
    ...
    if( $doAlign )
    {
        ...
        foreach my $type ( keys( %{ $erefs } ) )
        {
            ...
            while( my $line = <$sfh>)
            {
                ...
                if( $line =~ /^>(\w+)/ )
                {
                    print $tfh qq[>$1\n]; # NOT call them all by the type label
                }
                else
                {
                    print $tfh qq[$line\n];
        ...
        my $exo_file = './exonerate.PE.out';
        my $pm = Parallel::ForkManager->new(10);
        ALIGN_LOOP:
        for ($i=1; $i<=10; $i++)
           {
            my $pid = $pm->start and next ALIGN_LOOP;
            system("exonerate --model affine:local --bestn 5 --ryo \"INFO: %qi %qal %pi %tS %ti %qab %qae %tab %tae %qs\\n\" $candidatesFasta $refsFasta --querychunkid $i --querychunktotal 10 > ./exonerate.PE$i.out");
            $pm->finish; 
           }
        $pm->wait_all_children;
        system("cat ./exonerate.PE1.out ./exonerate.PE2.out ./exonerate.PE3.out ./exonerate.PE4.out ./exonerate.PE5.out ./exonerate.PE6.out ./exonerate.PE7.out ./exonerate.PE8.out ./exonerate.PE9.out ./exonerate.PE10.out > $exo_file");
        print qq[Parsing PE alignments....\n];
        my $lastLine;
        my ($anno, %anchors, %anno, @anno, %uniq, $hit, $line, $anchor_i, $input);
        my ($sa_read, $sa, %sa_tag);
        my $sa_file = qq[$$.SAtag];
        (open (SA , "<$sa_file ")) || die "cannot open the SA file\n";
        (open (EXO, "<$exo_file")) || die "cannot open the exo file\n";

        while ($line = <SA>)
           {
            chomp($line);
            ($sa_read, $sa) = split("\t", $line);
            $sa_tag{$sa_read} = $sa;
           }
        ...
        while ($line = <EXO>)
           {
            ...
            if ($line =~ /^INFO/)
               {
                $input = $line;
                while ( ($line = <EXO>) && ($line =~ /^[ACTGN]+/) && ($line !~ /^C4/) )
                  {
                   chomp($line);
                   $input .= $line;
                  }
        ...
        for $hit(keys %uniq)
           {
            ...
            if( $s[ 3 ] >= $id && $s[ 2 ] >= $length )
              {
               my @temp = split('###', $s[1]);
               $sa = exists($sa_tag{$s[1]}) ? $sa_tag{$s[1]} : 'SA_NA'; 
               $anchors{ $temp[ 0 ] }{$temp[1]."\t".$temp[2]."\t".$temp[4]} = 1; 
               $anno = $s[ 5 ]."\t".$s[ 4 ]."\t".$s[ 3 ]."\t".$s[ 6 ]."\t".$s[ 7]."\t".$s[ 8 ]."\t".$s[ 9 ]."\t".$s[ 10 ]."\t".$temp[ 3 ]."\t".$temp[ 4 ]."\t".$sa."\t".$s[11]; 
#TE type, mate orientation, percent ID. This could be a memory issue (possibly dump out to a file and then use unix join to intersect with the bed)
               $anno{$temp[0]."\t".$temp[1]."\t".$temp[2]."\t".$temp[4]}{$anno} = 1; 
              }
          }
        undef( %uniq );
        ...
        while( my $anchor = <$cfh> )
        {
            ...
            my @s = split( /\t/, $anchor );
            my $temp = $s[0]."\t".$s[1]."\t".$s[8];
            for $site(keys %{$anchors{$s[3]}})
            {
                next if ($temp eq $site);
                for $anno(keys %{$anno{$s[3]."\t".$site}})
                {
                 @anno = split("\t", $anno);
                 my $mate_orientation = $anno[ 1 ];
                 my $type = $anno[ 0 ];
                 my $rev_strand = ($anno[9] & $$BAMFLAGS{'reverse_strand'}) ? '-' : '+';
                #note the anchor orientation is contained is 3rd last, then mate orientation 2nd entry, and then percent id is last
                 print $afh qq[$s[0]\t$s[1]\t$s[2]\t$type\t$s[3]\t$s[4]\t$mate_orientation\t$anno[ 2 ]\t$anno[3]\t$anno[4]\t$anno[5]\t$anno[6]\t$anno[7]\t$anno[8]\t$anno[9]\t$rev_strand\t$s[5]\t$s[6]\t$s[7]\t$s[8]\t$s[9]\t$anno[10]\t$s[10]\t$s[11]\t$s[12]\t$anno[11]\n];
                }
            }
        }
        close( $afh );close( $cfh );
        ...
        if( $srmode && -s $clipFasta )
           {
            my $exo_file = './exonerate.SR.out';
            my $pm = Parallel::ForkManager->new(10);
            ALIGN_LOOP:
            for ($i=1; $i<=10; $i++)
               {
                my $pid = $pm->start and next ALIGN_LOOP;
                system("exonerate --model affine:local --bestn 5 --ryo \"INFO: %qi %qal %pi %tS %ti %ql %qab %qae %tab %tae %tas %qas %qs\\n\" $clipFasta $refsFasta --querychunkid $i --querychunktotal 10 > ./exonerate.SR$i.out");
                $pm->finish;
               }
            $pm->wait_all_children;
            system("cat ./exonerate.SR1.out ./exonerate.SR2.out ./exonerate.SR3.out ./exonerate.SR4.out ./exonerate.SR5.out ./exonerate.SR6.out ./exonerate.SR7.out ./exonerate.SR8.out ./exonerate.SR9.out ./exonerate.SR10.out > $exo_file");

            print qq[Parsing split-read alignments....\n];
            open( my $cofh, qq[>$srOutput] ) or die qq[Failed to create clipped candidates output file: $!\n];
            my $lastLine = '';
            my (%uniq, $anchor_i, $anchor_info, $first, %clipAnchor, $input, $hit, $line, %readsAligned, @data);

            (open (EXO, "<$exo_file")) || die "cannot open the exo_file\n";
            (open (CAN, "<$clipAnchor")) || die "cannot open the CAN file\n";

            while ($line = <CAN>)
              {
               chomp($line);
               @data = split("\t", $line);
               $first =  ($data[1] & 0x40) ? 0 : 1;
               $clipAnchor{$data[0]."\t".$first} = $data[1]."\t".$data[2]."\t".$data[3]."\t".$data[4]."\t".$data[5]."\t".$data[6]."\t".$data[7];
              }
                ...
                if( $s[ 3 ] >= $id && $s[ 2 ] >= $length )
                   {
                    ...
                    $first = ($readDetails[3] & 0x40) ? 1 : 0;
                    $anchor_info = exists($clipAnchor{$readDetails[0]."\t".$first}) ? $clipAnchor{$readDetails[0]."\t".$first} : 'NA';
                     #               chr                pos              pos          TE     readname            flag             cigar(blast)	align_strand	mappability	insertSize	qlenth	qab	qae	tab	tae	targetseq
                    my $print = qq[$readDetails[1]\t$readDetails[2]\t$readDetails[2]\t$s[5]\t$readDetails[0]\t$readDetails[3]\t$readDetails[4]\t$s[4]\t$s[3]\t$readDetails[5]\t$s[6]\t$s[7]\t$s[8]\t$s[9]\t$s[10]\t$s[11]\t$s[12]\t$s[13]\t$anchor_info\t$s[14]\n];
                    print $cofh $print;
...
sub _getCandidateTEReadNames
{
    ...
    my ($i1, $i2, $i3, $Match, $match_line);
    my ($first, $middle, $second, $newcigar, $replace_cigar);
    my ($refgenome, $efh, $chr, $leng);
    my ($line, @data, %refseq, %size, $seq);
    ...
    while ( my $samLine = <$bfh> )
    {
        ...
        my $pos = $sam[ 3 ];
        my $cigar = $sam[ 5 ];
        my $readLen2 = _getRightmostCord($pos, $cigar);
        my $sam_i = 0; 
        my $xa = 'XA:NULL';
        my $sa = 'SA_NA';
        my $as = 0;
        ...
        my $nm = 0;
        my $anchor_seq;
        for ($sam_i=10; $sam_i <=$#sam; $sam_i++)
            {
             $xa = $sam[$sam_i] if ($sam[$sam_i] =~ /^XA/);
             $sa = $1 if ($sam[$sam_i] =~ /^SA:Z:(.+)/);
             $as = $1 if ($sam[$sam_i] =~ /^AS:i:(.+)/);
             $xs = $1 if ($sam[$sam_i] =~ /^XS:i:(.+)/);
             if ($sam[$sam_i] =~ /^NM\:i\:(\d+)/)
                {
                 $nm = $1;
                }
        ...
        if( $candidates{ $name } )
        {
            if( ($flag & $$BAMFLAGS{'1st_in_pair'}) && $candidates{ $name } == 1 || ($flag & $$BAMFLAGS{'2nd_in_pair'}) && $candidates{ $name } == 2 )
            {
                ...
                if( _sequenceQualityOK( $seq ) )
                {
                 if ($sa ne 'SA_NA')
                    {
                     print SA "$name###$sam[2]###$sam[3]###$sam[5]###$sam[1]\t$sa\n";
                    }
                    if( defined($candidatesFasta) ){print $ffh qq[>$name###$sam[2]###$sam[3]###$sam[5]###$sam[1]\n$seq\n];}
                    
                    #record the read position details in the BED file of discordant mates
                    my $dir = ($flag & $$BAMFLAGS{'reverse_strand'}) ? '-' : '+';
                    print $dfh qq[$ref\t$pos\t].($pos+$readLen2).qq[\t$name\t$dir\t$qual\n];
                }
                delete( $candidates{ $name } );
            }
        }
        ...
        if( $supporting > 0 )
        {
            if( $supporting == 1 ) #single-ended mapped
            {
               ...
               my $dir = ($flag & $$BAMFLAGS{'reverse_strand'}) ? '-' : '+';
               print $afh qq[$ref\t$pos\t].($pos+$readLen2).qq[\t$name\t$dir\t$cigar\t$xa\t$nm\t$flag\t$anchor_seq\t$qual\t$as\t$xs\n];
               print $sebfh qq[$ref\t$pos\t].($pos+$readLen2).qq[\t$name\t$dir\n] if( $singleEndsBed );
            }
            ...
            if( $minSoftClip && ! ( $flag & $$BAMFLAGS{'unmapped'} ) && ! ( $flag & $$BAMFLAGS{'mate_unmapped'} ) && ( $mref eq '=' || $mref eq $ref ) && $sam[ 8 ] < 3000 ) 
            {
             my $seq = $sam[ 9 ];
             my $quals = $sam[ 10 ];
             ...
             if( $samLine =~ /NM\:i\:(\d+)/)
                {
                 my $md = $1;
                 my $mismatches = $md; #( $md =~ tr/ACGT/n/ );
                
                 if( $mismatches <= 3 )
                   {
                    $Match = 0;
                    #next if (! ( (($cigar=~tr/S/S/) == 1) && ($cigar =~ /(\d+)(S)/) && ($1 > $minSoftClip ) ) );
                    next if ( (($cigar=~tr/S/S/) == 0) || (($cigar=~tr/S/S/) >= 3) );
                    #clip at start of alignment
                    if ( $cigar =~ /^(\d+)S/ )
                       {
                        my $clip = $1;
                        next if ($clip <= $minSoftClip);
