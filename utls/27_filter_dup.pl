...
my $disc_file = $path.'/'.$retro.'/'.$sub.'.'.$te.'.alignfilter.discover';
...
(open (DISC, "<$disc_file")) || die "cannot open the disc file $disc_file\n";
        ...
        for ($i=0; $i<($count{$id}{$subte}-1); $i++)
           {
            for ($j=$i+1; $j<$count{$id}{$subte}; $j++)
               {
                $same1 = $same2 = 0;
                next if ($site{$id}{$subte}[$i][5] eq $site{$id}{$subte}[$j][5]);
                next if ($site{$id}{$subte}[$i][0] ne $site{$id}{$subte}[$j][0]);
                $same3 = 1; #&parse_anchor($site{$id}{$subte}[$i][6], $site{$id}{$subte}[$j][6]);
                $same1 ++ if ( (abs($site{$id}{$subte}[$i][2] - $site{$id}{$subte}[$j][2]) < 5) || (abs($site{$id}{$subte}[$i][1] - $site{$id}{$subte}[$j][1]) < 5) );  ### hg coord ###
                $same2 ++ if ( (abs($site{$id}{$subte}[$i][4] - $site{$id}{$subte}[$j][4]) < 5) || (abs($site{$id}{$subte}[$i][3] - $site{$id}{$subte}[$j][3]) < 5) );  ### TE coord ###
               #if ( ($site{$id}{$subte}[$i][1] =~ /337970/)  && ($site{$id}{$subte}[$j][1] =~ /337970/) )
               #   {
               #    print "$same1\t$same2\t$same3\n";
               #    print "$site{$id}{$subte}[$i][3]\t$site{$id}{$subte}[$j][3]\n";
               #    print "$site{$id}{$subte}[$i][4]\t$site{$id}{$subte}[$j][4]\n";
               #   }
               if ($same1 && $same2 && $same3)
                  {
                   $dup{$site{$id}{$subte}[$i][5]} = 1;   ### read name is an duplicate ###
                   $dup{$site{$id}{$subte}[$j][5]} = 1;
                   $pair{$site{$id}{$subte}[$i][5]}{$site{$id}{$subte}[$j][5]} = 1; ### pairs of all duplcates ###
...
for ($j=0; $j<=$#call; $j++)
   {
    ...
    if ($data[4] < 2)
       ...
       {
        print NEW "$data[0]\t$data[1]\t$data[2]\t$data[3]";
        $count=0;
        for ($i=5; $i<=$#data; $i++)
           {
