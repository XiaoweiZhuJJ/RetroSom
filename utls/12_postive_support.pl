...
my $retro = $ARGV[2];
my $TE = 'LINE';
my $sr_file = $path.'/'.$retro.'/'.$TE.'/'.$sub.'.sr2.pred.summary';
my $pe_file = $path.'/'.$retro.'/'.$TE.'/'.$sub.'.pe2.pred.summary';
...
while ($line = <PEA>)
   {
    ...
    if ( ($data[3] =~ /L1/) && (exists($pe{$r1.':'.$data[4]})) )
       {
        print PEO "$line\n";
       }
   }
