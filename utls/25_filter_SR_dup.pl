...
my $sr_file    = $ARGV[1].'/'.$ARGV[2].'/'.$ARGV[0].'_Model.'.$ARGV[3].'.sr.tabe.discover';
my $ou_file    = $ARGV[1].'/'.$ARGV[2].'/'.$ARGV[0].'_Model.'.$ARGV[3].'.sr.dedup.discover';
my $dup_file   = $ARGV[1].'/'.$ARGV[2].'/'.$ARGV[0].'_Model.'.$ARGV[3].'.sr.dup.txt';
my $clust_file = $ARGV[1].'/'.$ARGV[2].'/'.$ARGV[0].'_Model.'.$ARGV[3].'.sr.cluster.txt';
...
if ((abs($data1[1] - $data2[1])<5) || (abs($data1[2] - $data2[2])<5))
   { ### anchor coordinates ###
    $flag1 = 1;
   }
else
...
$flag3 = (abs($data1[4] - $data2[4]) < 5) ? 1 : 0;
$flag4 = (abs($data1[5] - $data2[5]) < 5) ? 1 : 0;
$flag5 = 0;
$flag6 = 1;
    ...
    if ($data2[6] =~ /(\d+)M(\d+)S/)
       {
        $c3 = $1;
        $c4 = $2;
        $flag5 = 1 if ( (abs($c1 - $c3)<5) || (abs($c2 - $c4) <5) );
       }
   }
